...
def load_oc(file):
    """Carga OC manteniendo la interfaz original: devuelve (oc, map_oc).

    Posiciones confirmadas por el usuario:
      G  (6)  = F. Docum.       fecha de OC
      AQ (42) = Material        código/material
      AU (46) = Unidades        cantidad comprada
      BJ (61) = Fecha Guia      fecha de recepción
      Lead Time = BJ - G
    """
    import pandas as pd

    d = pd.read_excel(file, dtype=object)

    pos = {"f_docum": 6, "material": 42, "unidades": 46, "fecha_guia": 61}
    if len(d.columns) <= max(pos.values()):
        raise ValueError(
            f"El archivo OC tiene {len(d.columns)} columnas; "
            f"se necesitan al menos {max(pos.values()) + 1}."
        )

    out = pd.DataFrame(index=d.index)
    out["material"] = d.iloc[:, pos["material"]].astype("string").str.strip()
    out["f_docum"] = pd.to_datetime(
        d.iloc[:, pos["f_docum"]], errors="coerce", dayfirst=True
    )
    out["unidades"] = pd.to_numeric(
        d.iloc[:, pos["unidades"]], errors="coerce"
    ).fillna(0)
    out["fecha_guia"] = pd.to_datetime(
        d.iloc[:, pos["fecha_guia"]], errors="coerce", dayfirst=True
    )

    # Lead Time: fecha de recepción (BJ) menos fecha OC (G).
    out["lead_time"] = (out["fecha_guia"] - out["f_docum"]).dt.days
    out.loc[(out["lead_time"] < 0) | (out["lead_time"] > 365), "lead_time"] = pd.NA

    # Clave interna para cruzar; NO modifica el código original mostrado.
    out["material_key"] = (
        out["material"].astype("string")
        .str.replace(r"\.0$", "", regex=True)
        .str.strip()
    )

    out = out[
        out["material_key"].notna()
        & (out["material_key"] != "")
        & (out["material_key"].str.lower() != "nan")
    ].copy()
    out.reset_index(drop=True, inplace=True)

    # Compatibilidad con app.py existente:
    # app.py espera: oc, map_oc = load_oc(f_oc)
    map_oc = out.copy()
