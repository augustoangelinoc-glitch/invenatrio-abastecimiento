...
def load_oc(file):
    """Carga OC usando las posiciones confirmadas:
       G=6 F. Docum., AQ=42 Material, AU=46 Unidades, BJ=61 Fecha Guia.
    """
    import pandas as pd
    ...
    need = max(pos.values())
    if len(d.columns) <= need:
        raise ValueError(
            f"El archivo OC tiene {len(d.columns)} columnas; "
            f"se necesitan al menos {need+1}."
        )
    ...
    # Clave de cruce: no altera el código que se muestra al usuario.
    out["material_key"] = (
