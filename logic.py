...
def load_oc(file):
    """Carga OC usando los nombres reales del archivo.

    Se evita depender de posiciones físicas, porque las versiones del reporte
    pueden insertar/eliminar columnas. Las columnas confirmadas son:
      F. Docum.  -> fecha de OC
      Material   -> código/material
      Unidades   -> cantidad comprada
      Fecha Guia -> fecha de recepción
      Estado     -> estado de la OC
    Lead Time = Fecha Guia - F. Docum.

    Devuelve exactamente (oc, map_oc), compatible con app.py.
    """
    d = pd.read_excel(file, dtype=object)

    fdoc = find_col(d, ["F. Docum.", "F.Docum.", "Fecha OC"])
    material = find_col(d, ["Material"])
    unidades = find_col(d, ["Unidades", "Cantidad"])
    guia = find_col(d, ["Fecha Guia", "Fecha Guía", "Fecha Guia "])

    estado_col = None
    for c in d.columns:
        if norm(c) == "ESTADO ITEM":
            estado_col = c
            break

    out = pd.DataFrame(index=d.index)
    out["material"] = code_text(d[material])
    out["key"] = out["material"].map(code_key)
    out["f_docum"] = pd.to_datetime(d[fdoc], errors="coerce", dayfirst=True)
    out["cantidad"] = numeric(d[unidades])
    out["fecha_guia"] = pd.to_datetime(d[guia], errors="coerce", dayfirst=True)
    ...
    if estado_col is not None:
        out["estado"] = code_text(d[estado_col])
    else:
        out["estado"] = ""
    ...
    map_oc = {
        ...
        "Fecha Guia": str(guia),
        "Estado Item": str(estado_col) if estado_col is not None else "NO ENCONTRADA",
        "Lead Time": "Fecha Guia - F. Docum.",
    }
    return out, map_oc
